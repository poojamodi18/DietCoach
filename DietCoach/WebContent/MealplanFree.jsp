...
// 										});
						...
						//	console.log('RANDOM NUMBER :: ' + randomIn);
							try {
							...
							for(var k=0;k<sr.length; k++){
							    jQuery.each(arr, function(x, val) {
										if(val.foodid==sr[k]){
											$(id).append("<tr data-id='"+val.foodid+"' data-carbs='"+val.carbs+"' data-protein='"+val.protein+"' data-sugar='"+val.sugar+"' data-fat='"+val.fat+"' data-fiber='"+val.fiber+"'><td><img alt='' src='data:image/jpeg;base64,"+val.foodImageString+"' height='80px' width='80px' style='border-radius: 8px;'></td> <td rel='tooltip' data-html='true' title='<h5>Nutition Value</h5><hr><table><tr><td>Calories</td><td>"+val.calories+"</td></tr><tr><td>Carbs</td><td>"+val.carbs+"g</td></tr><tr><td>Protein</td><td>"+val.protein+"g</td></tr><tr><td>Fat</td><td>"+val.fat+"g</td></tr><tr><td>Sugar</td><td>"+val.sugar+"g</td></tr><tr><td>Fiber</td><td>"+val.fiber+"g</td></tr></table>' data-placement='right'>"+val.foodname+"</td><td>Quantity : "+val.quantity+"&nbsp;"+val.servingType+"</td> <td class='text-right'></button> <button type='button' rel='tooltip' title='Recipe' data-placement='right' class='btn btn-primary btn-link ' data-foodId='"+val.foodrecipeId+"' data-toggle='modal' data-target='.bd-example-modal-lg'><i class='material-icons'>receipt_long</i></button></td></tr>");
											carbs+=val.carbs;
											protein+=val.protein;
											sugar+=val.sugar;
											fat+=val.fat;
...
// 				$.cookie('breakfast',breakfastCookie);
		}
